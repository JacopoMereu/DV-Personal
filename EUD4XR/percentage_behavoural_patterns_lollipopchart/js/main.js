...
function get_data() {
    const P = data => {
        return data.map(d => ({...d, Percentage: +d.Percentage}))
...
const margin = {top: 60, right: 50, bottom: 30, left: 160}
...
function render_data(data) {
    ...
    const key = 'Type', value = 'Percentage', sort = true
    render_lollipop_chart(svg, data, key, value, sort);
}

function render_lollipop_chart(svg, data, key, value, sort) {
    const first_items = ["Voice Only", "None"]

    console.log("DATA", data)
    if (sort)
        data.sort((a, b) => {
            if (first_items.includes(a[key])) return 1;
    ...
    gContainer.selectAll('rect')
        ...
        .attr('fill', d=>first_items.includes(d.Type) ? 'orange': 'steelblue')
